...
namespace WechatWeb.App_Code
{
    public class XmlTools
    {
		...
		{
			string root = HttpContext.Current.Request.Url.Scheme + "://" + HttpContext.Current.Request.Url.Authority + HttpContext.Current.Request.ApplicationPath;
			if (root.Last() != '/') root += "/";
        ...
        {
            XmlToolsCom xtCom1 = new XmlToolsCom();
            return xtCom1.GetNodeText(nodeName);
        }
        ...
        /// <returns></returns>
        /// <summary> 
        public static List<string> GetNodeTextList(string nodeName)
        {
            XmlToolsCom xtCom1 = new XmlToolsCom();
            List<string> list = xtCom1.GetNodesText(nodeName);
            return list;
        }
         
        #region 将XML下对应节点转换成DataTable
        /// <summary>
        /// 将XML下对应节点转换成DataTable
        /// </summary>
        /// <param name="xPath">要转换的节点</param>
        /// <param name="element">XML文档</param>
        /// <returns>DataTable</returns>
        public static DataTable GetDataTableByXml(string xPath, XmlElement element)
        {
            ...
            try
            {
                ...
                if (nodeList.Count > 0)
                {
                    ...
                    foreach (XmlNode item in nodeList)
                    {
                        ...
                        {
                            row[i] = item.SelectSingleNode(dt.Columns[i].ColumnName).InnerText;
                        }
                        dt.Rows.Add(row);
                    }
                }
    ...
    public class XmlToolsCom
    {
        ...
        public XmlToolsCom(int fileIndex)
        {
            if (fileIndex == 1)
            {
                xmlPath = HttpRuntime.AppDomainAppPath + "\\Config\\Wechat.xml";
                pdoc.Load(xmlPath);
            }
        }
        ...
        public XmlToolsCom(string xmlString)
        {
        ...
        /// <returns></returns>
        private XmlNodeList GetXmlNodeList(string noteName)
        {
        ...
        /// <summary>
        /// 把指定节点下面的所有子节点的属性转化为DataTable(未完成)
        /// </summary>
        /// <param name="nodeName">节点名称</param>
        /// <returns>数据表</returns>
        /// Author FredJiang
        /// Create Date 2013年8月16日
        public DataTable GetXmlAttrbuteToTable(string nodeName)
        {
            DataTable dt = new DataTable();
            XmlNodeList xList = GetXmlNodeList(nodeName);
            if (xList.Count > 0)
            {
                foreach (XmlNode node in xList)
                {
        ...
        /// <summary>
        /// 把XML节点下面的子节点的属性及文本值转为DataTable
        /// </summary>
