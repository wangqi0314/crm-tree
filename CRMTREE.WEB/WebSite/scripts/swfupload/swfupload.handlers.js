...
function uploadSuccess(file, serverData)
{
    try
    {
        ...
        if (-1 == serverData) {
            top.$.alert('文件上传失败！', '操作提示');
            return false;
        }
        else if (-2 == serverData) {
            top.$.alert('请选择要上传的文件！', '操作提示');
            return false;
        }
        else if (-3 == serverData) {
            top.$.alert('文件上传异常！', '操作提示');
            return false;
        } else {
            $("#hdFileNmae").val(serverData);
            $("#ImagePreview").html("<img style=\"width:150px;height:150px;\" src=\"http://www.crmtree.com/upload/" + serverData + "\" />");
        }
