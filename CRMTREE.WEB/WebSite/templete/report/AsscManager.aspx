<%@ Page Language="C#" AutoEventWireup="true" CodeFile="AsscManager.aspx.cs" Inherits="templete_report_AsscManager" %>
...
<script type="text/javascript">
    ...
    var _s_url_assc = '/handler/Reports/AsscManage.aspx';
    ...
    _personal.validate = function () {
        return $.form.validate({
            //textbox: ['AU_Name', 'AU_ID_No', 'AU_Dr_Lic', 'DL_Rel']
            textbox: ['AU_Name']
        });
    };
    //取消验证
    ...
    _personal.set = function (personal) {
        $.form.setData_New('#frm_personal', personal);
        $("#AU_Name").textbox('setValue', personal.AU_Name);
        $("#AU_Password").textbox('setValue', personal.AU_Password);
        $("#AU_Username").textbox('setValue', personal.AU_Username);
        $("#Account_AU_Password").textbox('setValue', personal.AU_Password);
    };

    //获取
    _personal.get = function () {
        var data = $.form.getData_New("#frm_personal", ['AU_Gender', 'AU_Married']);
        return data;
    };
        ...
        function getHTML() {
            var language = {
                ...
                personal: {
                    ...
                    Rel: '<%=Resources.CRMTREEResource.em_personal_Rel %>',
                    Acc: '<%=Resources.CRMTREEResource.em_personal_Acc %>',
                    Cars: '<%=Resources.CRMTREEResource.em_personal_Cars %>'
                },
                account: {
...
+ '                        <table id="frm_personal"  class="form' + (_isEn ? '' : ' form_cn') + '" style="width:600px;" border="0" cellpadding="3" cellspacing="2">'
+ '                            <tr>'
+ '                                <td class="text">' + language.personal.name + '</td>'
+ '                                <td class="fluid">'
+ '                                    <input id="MAU_Code" type="hidden" value="0" />'
...
+ '                            </tr>'
+ '                            <tr>'
+ '                                <td class="text">' + language.personal.Rel + '</td>'
+ '                                <td>'
+ '                                    <input id="DL_Rel" class="easyui-combobox" />'
+ '                                </td>'
+ '                               <td class="text">' + language.personal.Acc + '</td>'
+ '                                <td>'
+ '                                    <input id="DL_Acc" class="easyui-combobox" />'
+ '                                </td>'
+ '                            </tr>'
+ '                            <tr>'
+ '                                <td class="text"><%=Resources.CRMTREEResource.AllCars%></td>'
+ '                                <td>'
+ '                                    <input id="DL_Type" type="checkbox" />'
+ '                                </td>'
+ '                            </tr>'
+ '                            <tr>'
+ '                                <td class="text">' + language.personal.Cars + '</td>'
+ '                                <td style="width:300px" colspan="5">'
+ '                                      <input id="DL_CI_Code" type="hidden" value="0" />'
+ '                                     <input id="DL_Cars" class="easyui-combobox" style="width:473px" />'
+ '                                </td>'
+ '                            </tr>'
//+ '                            <tr>'
...
+ '    </div>';
        ...
        //初始化
        (function Init() {
            if (_params.action !== "add" && !(_params.AU > 0)) {
        ...
        function Save() {
            ...
            var params = { action: 'Save_Assc', AU_Code: _params.AU, MAU_Code: _params.MAU };
            ...
            $.post(_s_url_assc, { o: s_params }, function (res) {
                $.mask.hide();
        ...
        function BindPersonalSelects() {
            $.getWords([44, 47, 55], function (data) {
                ...
                $.post(_s_url, { o: JSON.stringify({ action: 'Get_Messaging_Carriers' }) }, function (data) {
                    ...
                    //加载下拉列表数据
                    $.bindWords_New({
                        wordIds: [9, 31, 23, 1036, 2036, 3036]
                        , selects: [
                           ...
                           // , { selectId: 'AU_Industry', wordId: 18 }
                            , { selectId: 'DL_Rel', wordId: 2036 }
                            , { selectId: 'DL_Acc', wordId: 3036 }
                            //, { selectId: 'DL_Cars', wordId: 15 }
                        ]
                        , onLoad: function () {
                            $.post(_s_url_assc, {
                                o: JSON.stringify({
                                    action: 'Get_User_Groups',
                                    PUG: _params.UG > 0 ? _params.UG : 0
                                })
                            }, function (data) {
                                $("#AU_UG_Code").initSelect({
                                    data: data
                                })
        ...
        function BindCustomerInfo() {
            ...
            var params = { action: 'Get_AsscInfo', AU_Code: _params.AU, MAU_Code: _params.MAU };
            $.post(_s_url_assc, { o: JSON.stringify(params) }, function (res) {
                if (!$.checkResponse(res)) { return; }
                var d = 1;
                if (res.Categories) {
                    d = res.Categories.DL_Type;
                }
                ...
                var personal = $.extend({}, res.personal, res.Categories, res.dealerEmpl);
                _personal.set(personal);

                if (res.Categories) {
                    if (res.Categories.DL_CI_Codes) {
                        _idsArr = res.Categories.DL_CI_Codes.split(',');
                        if (_contains(_idsArr, '0'))
                        { _idsArr.remove('0'); }
                        if (_idsArr)
                        { $("#DL_Cars").combobox("setValues", _idsArr); }
                        else
                            $("#DL_Cars").combobox("setValues", "");
                    }
                    else {
                        $("#DL_Cars").combobox("setValues", "");
                    }
                ...
                if (d == 1) {
                    $("#DL_CI_Code").parent().parent().show();
                    $("#DL_Type").attr("checked", false);
                }
                else if (d == 0) {
                    $("#DL_CI_Code").parent().parent().hide();
                    $("#DL_Type").attr("checked", true);
                }
    ...
    $(function () { 
        ...
        $("#DL_CI_Code").parent().parent().show();
        var auid = GetQueryString("MAU");

        if (GetQueryString("Action") == 'Add' && GetQueryString("TI") == 'Per') {
            auid = GetQueryString("AU_Code");
            $("#MAU_Code").attr('value', auid);
        }

        if (lan == "zh-cn") //中文
        {
            $('#DL_Cars').combobox({
                url: '../../manage/car/GetCarInfo.ashx?_auid=' + auid + '',
                valueField: 'CI_Code',
                textField: 'CAR_CN',
                multiple: true,
                panelHeight: 'auto'
        ...
        //是否只显示为批阅
        $("#DL_Type").click(function () {
            //处理显示的为批阅信息
            if ($("#DL_Type").is(":checked")) {
                $("#DL_CI_Code").parent().parent().hide();
                var arrValues = new Array();
                var val = $("#DL_Cars").combobox("getData");
                for (var i = 0; i < val.length; i++) {
                    arrValues.push(val[i].CI_Code);
                }
                $("#DL_Cars").combobox("setValues", arrValues);
            } else {
                $("#DL_CI_Code").parent().parent().show();
                if (typeof (_idsArr) != "undefined")
                    $("#DL_Cars").combobox("setValues", _idsArr);
            }
        });
    })
